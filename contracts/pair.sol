...
contract MdexFactory is IMdexFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'MdexSwapFactory: IDENTICAL_ADDRESSES/相同的地址');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'MdexSwapFactory: ZERO_ADDRESS/零地址');
        require(getPair[token0][token1] == address(0), 'MdexSwapFactory: PAIR_EXISTS/对存在');
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'MdexSwapFactory: FORBIDDEN/禁止');
        feeTo = _feeTo;
    }

    function setFeeToSetter(address _feeToSetter) external {
        require(msg.sender == feeToSetter, 'MdexSwapFactory: FORBIDDEN/禁止');
        require(_feeToSetter != address(0), "MdexSwapFactory: FeeToSetter is zero address/FeeToSetter 是零地址");
        feeToSetter = _feeToSetter;
    }

    function setFeeToRate(uint256 _rate) external {
        require(msg.sender == feeToSetter, 'MdexSwapFactory: FORBIDDEN/禁止');
        require(_rate > 0, "MdexSwapFactory: FEE_TO_RATE_OVERFLOW/费率溢出");
        feeToRate = _rate.sub(1);
    }

    function setInitCodeHash(bytes32 _initCodeHash) external {
        require(msg.sender == feeToSetter, 'MdexSwapFactory: FORBIDDEN/禁止');
        require(initCode == false, "MdexSwapFactory: Do not repeat settings initCodeHash/不要重复设置initCodeHash");
        initCodeHash = _initCodeHash;
        initCode = true;
    }

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    // 返回排序的令牌地址，用于处理按此顺序排序的对的返回值
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure returns (uint amountB) {
        require(amountA > 0, 'MdexSwapFactory: INSUFFICIENT_AMOUNT/数量不足');
        require(reserveA > 0 && reserveB > 0, 'MdexSwapFactory: INSUFFICIENT_LIQUIDITY/流动性不足');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    // 给定资产的输入金额和对准备金，返回其他资产的最大输出金额
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public view returns (uint amountOut) {
        require(amountIn > 0, 'MdexSwapFactory: INSUFFICIENT_INPUT_AMOUNT/输入量不足');
        require(reserveIn > 0 && reserveOut > 0, 'MdexSwapFactory: INSUFFICIENT_LIQUIDITY/流动性不足');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public view returns (uint amountIn) {
        require(amountOut > 0, 'MdexSwapFactory: INSUFFICIENT_OUTPUT_AMOUNT/输出量不足');
        require(reserveIn > 0 && reserveOut > 0, 'MdexSwapFactory: INSUFFICIENT_LIQUIDITY/流动性不足');
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'MdexSwapFactory: INVALID_PATH/无效路径');
...
library SafeMath {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "SafeMath: addition overflow/SafeMath：加法溢出");

        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "SafeMath: subtraction overflow/SafeMath：减法溢出");
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        // 气体优化：这比要求 'a' 不为零要便宜，但是
        // 如果 'b' 也被测试，好处就会丢失。
        // 参见：https:github.com/OpenZeppelinopenzeppelin-contractspull522
        if (a == 0) {
            return 0;
        }

        uint256 c = a * b;
        require(c / a == b, "SafeMath: multiplication overflow/SafeMath：乘法溢出");

        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        return div(a, b, "SafeMath: division by zero/SafeMath：除以零");
    }
    ...
    function div(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        ...
        //断言（a == b c + a % b）；没有任何情况下这不成立
        return c;
    }

    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        return mod(a, b, "SafeMath: modulo by zero/SafeMath：以零为模");
    }
...
// 处理二进制定点数的库（https:en.wikipedia.orgwikiQ_(number_format)）
// range: [0, 2**112 - 1]
// resolution: 1 / 2**112
// 范围：[0, 2**112 - 1]
// 分辨率：1 /2**112
